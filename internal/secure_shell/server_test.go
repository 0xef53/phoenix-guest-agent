...
func TestServerShutdown(t *testing.T) {
	...
	go func() {
		...
		if err != nil && err != ErrServerClosed {
			t.Error(err) // Mark test as failed, but continue
			return       // Use return to stop this goroutine
		}
	}()
	...
	go func() {
		...
		if !bytes.Equal(stdout.Bytes(), testBytes) {
			t.Errorf("expected = %s; got %s", testBytes, stdout.Bytes()) // Mark test as failed, but continue
			return                                                       // Use return to stop this goroutine
		}
	}()
